...
- Catalog: the owner's 12 real products (her photos + Kurdish copy, translated to ar/en) across 3 categories — face-care, hair-care, tea-wellness (replaced the old lips-body). Images in `artifacts/mang-herbal/public/products/`; `lib/db/src/seed.ts` mirrors this catalog. Out-of-stock is enforced server-side (cart add + checkout return `OUT_OF_STOCK`) and shown as badges/toasts in the UI.
